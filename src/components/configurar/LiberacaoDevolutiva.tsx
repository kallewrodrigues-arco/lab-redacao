  ...
  return (
    ...
    >
      <span style={{
        ...
        {checked && (
          <span style={{ width: 12, height: 12, borderRadius: '50%', background: '#0467db' }} />
        )}
      </span>
    </button>
  );
...
export function LiberacaoDevolutiva({ colecoesAtivas, value, onChange }: LiberacaoDevolutivaProps) {
    ...
    <section style={{
      ...
      {colecoes.length === 0 ? (
        <p style={{ margin: 0, fontSize: 14, color: '#abb3c4', lineHeight: 1.5 }}>
          Nenhuma coleção ativa. Configure as turmas para definir a devolutiva.
        </p>
      ) : (
        <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap' }}>
          {colecoes.map((colecao) => {
            ...
            return (
              <div key={colecao} style={{ flex: '1 0 0', minWidth: 280 }}>
                <div style={{
                  background: 'white',
                  border: '1px solid #d2d9e5',
                  borderRadius: 16,
                  ...
                  <div style={{ width: 80, flexShrink: 0, position: 'relative', overflow: 'hidden' }}>
                    <img
                      ...
                      style={{
                        ...
                        objectFit: 'cover', pointerEvents: 'none', borderRadius: 4,
                      }}
                    />
                  </div>
                  ...
                  <div style={{
                    ...
                    {/* Título da coleção */}
                    <p style={{
                      margin: 0, fontSize: 20, fontWeight: 600, color: '#626c80',
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                      ...
                      >
                        <RadioButton checked={modoAtual === 'automatico'} onClick={() => select(colecao, 'automatico')} />
                        <div style={{
                          flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 8,
                          justifyContent: 'center', alignSelf: 'stretch', padding: '4px 0',
                        }}>
                          ...
                          <p style={{ margin: 0, fontSize: 14, fontWeight: 400, color: '#626c80', lineHeight: 1.25, letterSpacing: '0.2px' }}>
                            As devolutivas são liberadas para estudantes sem moderação do professor
                          </p>
                        </div>
                      </div>

                      {/* Moderado por professor */}
                      <div
                        style={{ display: 'flex', alignItems: 'flex-start', cursor: 'pointer' }}
                        onClick={() => select(colecao, 'manual')}
                      >
                        <RadioButton checked={modoAtual === 'manual'} onClick={() => select(colecao, 'manual')} />
                        <div style={{
                          flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 8,
                          justifyContent: 'center', alignSelf: 'stretch', padding: '4px 0',
                        }}>
                          ...
                          <p style={{ margin: 0, fontSize: 14, fontWeight: 400, color: '#626c80', lineHeight: 1.25, letterSpacing: '0.2px' }}>
                            O professor analisa e altera as devolutivas
                          </p>
                        </div>
                      </div>
