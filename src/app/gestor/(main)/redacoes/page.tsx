...
export default async function GestorRedacoesPage({ searchParams }: PageProps) {
  ...
  const groups = status
    ? null
    : {
        pendentes: redacoes.filter((r) => ['enviada', 'corrigindo', 'aguardando_liberacao'].includes(r.status)),
        corrigidas: redacoes.filter((r) => r.status === 'corrigida'),
        rejeitadas: redacoes.filter((r) => r.status === 'rejeitada'),
        outras: redacoes.filter((r) => ['pendente_envio', 'ilegivel'].includes(r.status)),
      };
    ...
    <div style={{ maxWidth: 960, margin: '0 auto' }}>
      ...
      {status ? (
        ...
        <>
          ...
          {groups!.rejeitadas.length > 0 && (
            <section style={{ marginBottom: 40 }}>
              <h2 style={{ margin: '0 0 16px', fontSize: 20, fontWeight: 600, color: 'var(--text-body)', letterSpacing: '-0.2px' }}>
                Rejeitadas
              </h2>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {groups!.rejeitadas.map((r) => (
                  <EssayListItem key={r.id} redacao={r} proposta={r.proposta} href={`/professor/propostas/${r.propostaId}?aba=correcao`} />
                ))}
              </div>
            </section>
